#MTT- build Report Modified Function
...
#' @param active.period The most common active period of fire spread. Will be used to select the interval of meteorological values to be used in the creation of clusters or percentiles. Options are: "energy", "all" and a numeric vector (e.g. c(11,15) - will use only the hours of 11 to 15). "energy" will use the hours 12 to 20 and "all" will use all the hours (00 to 23).
...
#' @param fire.aggregation How the meteorology of different points from the ERA5-Land grid that fall inside of the same fire perimeter should be combined. Possible methods include: WS (wind speed; the ERA5-Land grid point falling inside the fire perimeter with highest wind speed is kept and the others ignored), none (uses all the ERA5-Land grid points falling inside the fire perimeter).
...
#' create.clusters=TRUE,live.fuel.moisture=c(60,90),
...
packages <- c(
  "sf", "rgeos", "rgdal", "sp", "tibble", "tidyverse", "tseries", "ggpubr", 
  "ggplot2", "dplyr", "ggspatial", "rnaturalearth", "rnaturalearthdata", 
  "RColorBrewer", "factoextra", "car", "mclust", "officer", "flextable", 
  "magrittr", "tidyr", "maps", "scales","timetk","tidyquant"
)
...
build_report_Mod<- function(study.area, my.fires,my.dated.fires,meteo.data,active.period,meteo.aggregation,
  ...
  #mod------------
  my_fires_t_inter$Date_ini <- format(
    as.Date(my_fires_t_inter$Date_ini, format = "%Y/%m/%d"),
    "%Y-%m-%d"
  )  
  my_fires_t_inter$Date_end <- format(
    as.Date(my_fires_t_inter$Date_end, format = "%Y/%m/%d"),
    "%Y-%m-%d"
  )  
  #-------------
  ...
  #mod------------
  dated_fires$Date_ini <- format(
    as.Date(dated_fires$Date_ini, format = "%Y/%m/%d"),
    "%Y-%m-%d"
  )  
  dated_fires$Date_end <- format(
    as.Date(dated_fires$Date_end, format = "%Y/%m/%d"),
    "%Y-%m-%d"
  )  
  #-------------
  ...
  #mod------------
  #dated_fires_intersected$Data_ini <- format(
  ##  as.Date(dated_fires_intersected$Data_ini, format = "%Y/%m/%d"),
  #  "%Y-%m-%d"
  #)  
  #dated_fires_intersected$Data_end <- format(
  #  as.Date(dated_fires_intersected$Data_end, format = "%Y/%m/%d"),
  #  "%Y-%m-%d"
  #)  
  #-------------
  ...
  if (class(calibration_period_use)=="character"){
    if (calibration_period_use=="coincident"){
      ...
      final_period_analysis <- base::Reduce(intersect, list(period_size_dist_fire,period_dated_fire))
      final_period_analysis <- final_period_analysis[c(1,length(final_period_analysis))]
    }}
  ...
  dated_fires_join_use <- subset(dated_fires_join_use,Year >=final_period_analysis[1] & Year <= final_period_analysis[2])
  ...
  #Modification: dates for fires----------
  #dated_fires_join_use$Data_ini <- format(
  #  as.Date(dated_fires_join_use$Data_ini, format = "%Y/%m/%d"),
  #  "%Y-%m-%d"
  #)  
  #dated_fires_join_use$Data_end <- format(
  #  as.Date(dated_fires_join_use$Data_end, format = "%Y/%m/%d"),
  #  "%Y-%m-%d"
  #)
  #
  ...
  meteo_fires_inside$concat_use_fire_aggre <- paste0(meteo_fires_inside$ID,#meteo_fires_inside$row,meteo_fires_inside$col,
                                                     meteo_fires_inside$day_use)
  ...
  meteo_fires_inside_use_for_aggregation <- meteo_fires_inside[,c("ID", "row", "col", "lon",  "lat","concat_use","concat_use_fire_aggre")]
  ...
  if(meteo.aggregation=="mean") {
    ...
    meteo_fires_inside1 <- left_join(meteo_fires_inside1,meteo_fires_inside_use_for_aggregation, by="concat_use",multiple="all")
  ...
  if(missing(summarize.per.fire)){
    summarize.per.fire <- FALSE
  }
  ...
  if (my.fires!=my.dated.fires){
    ...
    description_table_FS_meteo_data <- data.frame(round(min(dated_fires_join_use$Area_ha),0),
                                                  round(max(dated_fires_join_use$Area_ha),0),
                                                  #median(my_fires_t_inter_intersected_all_fires$Area_ha),
                                                  round(mean(dated_fires_join_use$Area_ha),0),
                                                  round(sd(dated_fires_join_use$Area_ha),0),
                                                  round(as.numeric(quantile(dated_fires_join_use$Area_ha, prob=c(.25))),0),
                                                  round(as.numeric(quantile(dated_fires_join_use$Area_ha, prob=c(.5))),0),
                                                  round(as.numeric(quantile(dated_fires_join_use$Area_ha, prob=c(.75))),0),
                                                  round(as.numeric(quantile(dated_fires_join_use$Area_ha, prob=c(.9))),0))
  ...
  for_wind_rose_0ha <- subset(dated_fires_join_use,area_correct>=0)
  ...
  for_wind_rose_100ha <- subset(dated_fires_join_use,area_correct>=100)
  ...
  for_wind_rose_500ha <- subset(dated_fires_join_use,area_correct>=500)
  ...
  for_wind_rose_1000ha <- subset(dated_fires_join_use,area_correct>=1000)
  ...
  if (create.clusters == TRUE){
    ...
    my_cluster_files <- list.files(
      path = mainDir,
      pattern = "(clusters|clustering)\\.csv$",
      full.names = F
    )    
    #setwd(file.path(mainDir,subDir))
    ...
    for(q in 1:length(my_cluster_files)){
      ...
      my_cluster_loop$HTR_1_forests <- round(((0.1617-0.001419*my_cluster_loop$T)*(-log(1-my_cluster_loop$RH/100))^(0.4657-0.003578*my_cluster_loop$T))*100,0)
      ...
      my_cluster_loop$HTR_1_shrubs <- round((4.37+0.161*my_cluster_loop$RH)-0.1*(my_cluster_loop$T -25)-0.027*my_cluster_loop$RH,0)
