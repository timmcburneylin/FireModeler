...
#' @param ECMWF_user Personal ECMWF userID token. Can be retrieved from the user profile page after logging in at https://cds.climate.copernicus.eu/profile
...
GetFireWeather_Mod <- function(study.area, my.fires,data.source,output.folder,utc.zone,ECMWF_user,ECMWF_API,user_email) {
  ...
  my_fires_t <- my_fires_t %>%
    group_by(ID) %>%
    slice_max(Date_ini, n = 1, with_ties = FALSE) %>%
    ungroup()
  
  #modification 1-------------------------------------------------
  my_fires_t$Date_ini <- format(
    as.Date(my_fires_t$Date_ini, format = "%Y/%m/%d"),
    "%Y-%m-%d"
  )  
  my_fires_t$Date_end <- format(
    as.Date(my_fires_t$Date_end, format = "%Y/%m/%d"),
    "%Y-%m-%d"
  )
  #-----------------------------------------------------------
  ...
  wf_set_key(key=ECMWF_user, user = "ecmwfr")
  ...
  if (data.source=="era5-land") {
    ...
    for (w in 1:n_times_required){
      ...
      my_year <- unique(format(as.Date(days_fire_final_loop, format="%Y-%m-%d"),"%Y"))
      my_month <- unique(format(as.Date(days_fire_final_loop, format="%Y-%m-%d"),"%m"))
      my_day <- unique(format(as.Date(days_fire_final_loop, format="%Y-%m-%d"),"%d"))
    ...
    foreach(w=1:length(list_requests), .packages = c("ecmwfr")) %dopar% {
      
      # Random delay before starting each request (0-30 seconds)
      ...
      while (!success && retry_count < max_retries) {
        tryCatch({
          ...
          success <- TRUE
          cat(paste("Request", w, "completed successfully\n"))
          
        }, error = function(e) {
          if (grepl("rate limit|429", e$message, ignore.case = TRUE)) {
            retry_count <<- retry_count + 1
            wait_time <- min(60 * retry_count, 300)  # Progressive backoff: 60s, 120s, 180s, 240s, 300s max
            cat(paste("Rate limit hit for request", w, "- waiting", wait_time, "seconds (attempt", retry_count, "of", max_retries, ")\n"))
            Sys.sleep(wait_time)
          } else {
            # For non-rate-limit errors, fail immediately
            cat(paste("Non-rate-limit error for request", w, ":", e$message, "\n"))
            stop(e)
          }
        })
      }
      ...
      if (!success) {
        stop(paste("Request", w, "failed after", max_retries, "attempts"))
      }
  ...
  final_file<-my_nc_files[length(my_nc_files)]
  
  #create fake file if missing--------------
  extract_number <- function(filename) {
    num_str <- gsub(".*area_(\\d+)\\.nc$", "\\1", filename)
    return(as.numeric(num_str))
  }
  existing_numbers <- sapply(my_nc_files, extract_number)
  existing_numbers <- sort(existing_numbers)
  max_number <- max(existing_numbers)
  expected_sequence <- 1:max_number
  missing_numbers <- setdiff(expected_sequence, existing_numbers)
  
  if (length(missing_numbers) == 0) {
    print("✓ All files are present - no gaps in sequence!")
  } else {
    print(paste("✗ Missing", length(missing_numbers), "files:"))
    print(paste("Missing numbers:", paste(missing_numbers, collapse = ", ")))
    
    # Function to find the best source file for copying
    find_source_file <- function(target_num, existing_nums, max_search_distance = 5) {
      for (distance in 1:max_search_distance) {
        # Try files before the target
        candidate_before <- target_num - distance
        if (candidate_before %in% existing_nums) {
          source_file <- paste0("era5_weather_study_area_", candidate_before, ".nc")
          return(list(file = source_file, number = candidate_before, direction = "before"))
        }
        
        # Try files after the target
        candidate_after <- target_num + distance
        if (candidate_after %in% existing_nums) {
          source_file <- paste0("era5_weather_study_area_", candidate_after, ".nc")
          return(list(file = source_file, number = candidate_after, direction = "after"))
        }
      }
      return(NULL)  # No suitable source found
    }
    for (missing_num in missing_numbers) {
      target_file <- paste0("era5_weather_study_area_", missing_num, ".nc")
      
      # Find source file to copy
      source_info <- find_source_file(missing_num, existing_numbers)
      
      if (!is.null(source_info)) {
        source_file <- source_info$file
        direction <- source_info$direction
        source_num <- source_info$number
        
        print(paste("Missing:", target_file))
        print(paste("  → Copying from:", source_file, paste0("(", direction, ", distance: ", abs(missing_num - source_num), ")")))
        
        # Check if source file exists before copying
        if (file.exists(source_file)) {
          # Copy the file
          file.copy(from = source_file, to = target_file)
          
          # Verify the copy was successful
          if (file.exists(target_file)) {
            print(paste("  ✓ Successfully created:", target_file))
          } else {
            print(paste("  ✗ Failed to create:", target_file))
          }
        } else {
          print(paste("  ✗ Source file does not exist:", source_file))
        }
      } else {
        print(paste("  ✗ No suitable source file found for:", target_file))
      }
    }
  }
  ...
  for(z in 1:length(my_nc_files)){
    ...
    if (z == 1){
      ...
      lat <- round(lat,1)
      lon <- round(lon,1)
      #modification 2: adjust for "valid_time"--------------------
      dims<-names(my_data_temp$dim)
      time_name<- if("time" %in% dims){
        "time"
      }else if("valid_time" %in% dims){
        "valid_time"
      }else{
        NA
      }
        
      t <- ncvar_get(my_data_temp, time_name)
      
      units_str <- ncatt_get(my_data_temp, time_name, "units")$value
      origin    <- sub(".*since\\s+", "", units_str)
      
      timestamp_use <- as_datetime(c(t*60*60),origin=origin)
      timestamp_use <- as_datetime(c(t),origin=origin)
      ...
      timestamp_use_z <- as_datetime(c(t),origin=origin)
      #-----------------------------------------------
  ...
  for (i in 1:nrow(my_fires_t_dated_2020)){
    ...
    #  }
      ...
      # Modification:fix arrary row error------------------------------
      for(l in 1:nrow(results_subset_my_points)){
        ...
        # Extract weather data with proper dimension handling
        WS.array_loop_data <- round(as.vector(WS.array_loop[results_subset_my_points_loop$linha_mat, 
                                                            results_subset_my_points_loop$col_mat, ]), 2)
        WD.array_loop_data <- round(as.vector(WD.array_loop[results_subset_my_points_loop$linha_mat, 
                                                            results_subset_my_points_loop$col_mat, ]), 2)
        RH.array_loop_data <- round(as.vector(RH.array_loop[results_subset_my_points_loop$linha_mat, 
                                                            results_subset_my_points_loop$col_mat, ]), 2)
        t2m.array_loop_data <- round(as.vector(t2m.array_loop[results_subset_my_points_loop$linha_mat, 
                                                              results_subset_my_points_loop$col_mat, ]), 2)
        
        # Handle length mismatch between extracted data and expected files
        expected_length <- length(files_to_get)
        actual_length <- length(t2m.array_loop_data)
        
        if(actual_length != expected_length) {
          if(actual_length < expected_length) {
            
            # Pad with NAs if data is shorter than expected
            pad_length <- expected_length - actual_length
            t2m.array_loop_data <- c(t2m.array_loop_data, rep(NA, pad_length))
            WS.array_loop_data <- c(WS.array_loop_data, rep(NA, pad_length))
            WD.array_loop_data <- c(WD.array_loop_data, rep(NA, pad_length))
            RH.array_loop_data <- c(RH.array_loop_data, rep(NA, pad_length))
          } else {
            # Truncate if data is longer than expected
            t2m.array_loop_data <- t2m.array_loop_data[1:expected_length]
            WS.array_loop_data <- WS.array_loop_data[1:expected_length]
            WD.array_loop_data <- WD.array_loop_data[1:expected_length]
            RH.array_loop_data <- RH.array_loop_data[1:expected_length]
          }
        }
        
        # Find insertion point in result_hours
        NAindex <- which(is.na(result_hours))
        if(length(NAindex) == 0) {
          stop("result_hours matrix is full - need to expand it")
        }
        firstNA <- min(NAindex)
        
        start_results <- firstNA
        end_results <- start_results + length(t2m.array_loop_data) - 1
        ...
        # Create tables for repeated values
        data_length <- length(t2m.array_loop_data)
        id_use_table <- rep(results_subset_my_points$ID[l], data_length)
        linha_mat_table <- rep(results_subset_my_points$linha_mat[l], data_length)
        col_mat_table <- rep(results_subset_my_points$col_mat[l], data_length)
        long_table <- rep(results_subset_my_points$long[l], data_length)
        lati_table <- rep(results_subset_my_points$lati[l], data_length)
        
        # Safe assignment to result_hours
        result_hours[start_results:end_results, 1] <- id_use_table
        result_hours[start_results:end_results, 2] <- linha_mat_table
        result_hours[start_results:end_results, 3] <- col_mat_table
        result_hours[start_results:end_results, 4] <- round(as.numeric(long_table), 2)
        result_hours[start_results:end_results, 5] <- round(as.numeric(lati_table), 2)
        result_hours[start_results:end_results, 6] <- as.numeric(old_date_hours[1:data_length])
        result_hours[start_results:end_results, 7] <- as.numeric(new_date_hours_vs2[1:data_length])
        result_hours[start_results:end_results, 8] <- t2m.array_loop_data  # Fixed line!
        result_hours[start_results:end_results, 9] <- RH.array_loop_data
        result_hours[start_results:end_results, 10] <- WS.array_loop_data
        result_hours[start_results:end_results, 11] <- WD.array_loop_data
      }
