...
snapshot_every = 60   # seconds (1 minute)
t_end = 300          #total sim time [s]
...
Montilla_FireDynamics<- function(
  ...
  # ---------- helpers (added, use your naming everywhere else) ----------
  ...
  if(Spatial == TRUE){
    ...
    # -------- NEW: build cropped fine template & downscale to 'cellsize' --------
...
testresults<-Montilla_FireDynamics(Spatial=TRUE,
                      Landscape<-Landscape_c,
                      Environment<-Enviroment_c,
                      Weather,
                      Ignitions=NULL,
