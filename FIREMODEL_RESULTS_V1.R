...
#Weather System:
NumWeathers=250 #userinput #Number of weather days to iterate over: choose between 100-500 to avoid long processing times, make shorter for more TUs
...
Res <- rbindlist(
  lapply(names(Results), function(name) {
    ...
    period_match  <- str_match(name, "^(.*?)__+Strata")[,2]
...
#FBP at 90th:-------------------------------------------------
Wthr90<-read.csv(paste0(path,out_weather,"processed/",WthrName,"_Percentiles.csv"))
...
FBP.res<-cffdrs::fbp(FBP_df,output = "ALL") %>% 
  mutate(Period=Structure$Period,Strata=Structure$Stratum,FireType=ifelse(FD == "C","Active Crown Fire",
         ...
         CSI=round(0.001*(Structure$FSGFINAL^1.5)*(460+25.9*FMC)^1.5,2),
         CSI_Met=FI<CSI,
